      ...
      style={{
        ...
        background: isDanger
          ...
          : isGhost
            ? "transparent"
            : isPrimary
              ? (disabled ? "rgba(244,180,0,0.06)" : "rgba(244,180,0,0.16)")
              : "transparent",
...
function ProfileCard({ profile, isActive, onSelect, onEdit, onDelete, selectLoading, isMobile }) {
  return (
    <motion.div
    ...
    >
      ...
      <motion.div
        ...
        whileTap={{ scale: 0.96 }}
        onClick={() => !selectLoading && onSelect(profile._id)}
        style={{
          ...
          background: isActive
            ...
            : "rgba(13,21,37,0.9)",
          border: `2px solid ${isActive ? "#F4B400" : "rgba(244,180,0,0.18)"}`,
          display: "flex", alignItems: "center", justifyContent: "center",
          fontSize: isMobile ? "1.4rem" : "1.7rem",
          cursor: selectLoading ? "wait" : "pointer",
          boxShadow: isActive ? "0 0 24px rgba(244,180,0,0.45), 0 0 48px rgba(244,180,0,0.15)" : "none",
...
function AddProfileCard({ onClick, isMobile }) {
  return (
    ...
    >
      <motion.div
        whileHover={{ scale: 1.06, borderColor: "rgba(244,180,0,0.5)" }}
        whileTap={{ scale: 0.96 }}
        onClick={onClick}
        style={{
          width: isMobile ? 56 : 68,
      ...
      <span style={{
        ...
        color: "rgba(232,217,160,0.3)", letterSpacing: "0.02em",
      }}>Add</span>
    </motion.div>
  );
}
    ...
    >
      ...
      >
        ...
        <h2 style={{
          ...
          letterSpacing: "0.18em", color: "#F4B400", marginBottom: "1.5rem",
        ...
        <HexInput
          ...
          placeholder="e.g. Aryan, Work, Study..."
        />
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
          ...
          <HexActionButton
            onClick={() => onSave({ name, avatar, profession })}
            loading={saving}
    ...
    >
      <motion.div
        ...
        style={{
          ...
          borderRadius: 6, padding: "2rem", width: "100%", maxWidth: 360,
          position: "relative",
        }}
      >
        <div style={{
        ...
        <h2 style={{
          ...
          letterSpacing: "0.15em", color: "#FF8080",
          textAlign: "center", marginBottom: "0.75rem",
        }}>DELETE PROFILE?</h2>
        <p style={{
          fontFamily: "Syne, sans-serif", fontSize: "0.82rem",
          color: "rgba(232,217,160,0.45)", textAlign: "center",
          marginBottom: "1.5rem", lineHeight: 1.6,
        }}>
          <span style={{ color: "rgba(255,150,150,0.7)" }}>{profile.name}</span> and all their
          chat history will be permanently deleted.
        </p>
        <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
          <HexActionButton variant="ghost" onClick={onClose}>Cancel</HexActionButton>
          <HexActionButton variant="danger" onClick={onConfirm} loading={deleting}>
            Delete
          </HexActionButton>
        </div>
      </motion.div>
...
export default function Profile() {
  ...
  const [selectLoading, setSelectLoading]   = useState(null); // profileId being selected
  const [modal, setModal]                   = useState(null); // { type: "create"|"edit"|"delete", profile? }
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setProfileList(profilesData.profiles || []);
  ...
  const handleCreateProfile = async ({ name, avatar, profession: prof }) => {
    ...
    try {
      ...
      setProfileList(prev => [...prev, data.profile]);
      setModal(null);
  ...
  const handleEditProfile = async ({ name, avatar, profession: prof }) => {
    ...
    try {
      ...
      setProfileList(prev => prev.map(p => p._id === data.profile._id ? data.profile : p));
      // Update localStorage if this is the active profile
  ...
  const handleDeleteProfile = async () => {
    ...
    try {
      ...
      setProfileList(prev => prev.filter(p => p._id !== modal.profile._id));
      if (wasActive) {
    ...
    return (
      <div style={{
        ...
        <motion.div
          ...
          style={{
            width: 40, height: 40 * 1.1547, clipPath: HEX,
            border: "2px solid rgba(244,180,0,0.15)",
            borderTopColor: "#F4B400", background: "transparent",
          }}
        />
      </div>
    );
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}
          ...
          style={{
            ...
            background: "rgba(244,180,0,0.07)", border: "1px solid rgba(244,180,0,0.2)",
            borderRadius: 3, padding: "6px 14px", cursor: "pointer",
            fontFamily: "Orbitron, sans-serif", fontSize: "0.55rem",
            letterSpacing: "0.15em", color: "rgba(244,180,0,0.6)",
      ...
      <div style={{
        ...
        padding: isMobile ? "5.5rem 1.25rem 3rem" : "6rem 1.5rem 3rem",
      }}>
        ...
        <motion.div
          ...
          style={{ marginBottom: "2rem", display: "flex", alignItems: "center", gap: 14 }}
        >
          <motion.div
          ...
          >
            <div style={{
              ...
              boxShadow: "0 0 28px rgba(244,180,0,0.4)",
            }}>👤</div>
          </motion.div>
          <div>
            <h1 style={{
              fontFamily: "Orbitron, sans-serif", fontWeight: 900,
              fontSize: isMobile ? "1.1rem" : "1.3rem", color: "#F4B400",
              letterSpacing: "0.12em", marginBottom: 4,
              textShadow: "0 0 30px rgba(244,180,0,0.3)",
            }}>HIVE PROFILE</h1>
            <p style={{
              ...
              {username && " · "}
              Manage your worker bee identity
            </p>
          </div>
        </motion.div>

        {/* ── Section 0: Hive Members ── */}
        <SectionCard title="Hive Members" icon="🐝" delay={0}>
          <p style={{
            fontFamily: "Syne, sans-serif", fontSize: "0.82rem",
            color: "rgba(232,217,160,0.4)", marginBottom: "1.5rem", lineHeight: 1.6,
          }}>
            Up to <span style={{ color: "#FFD54F" }}>6 members</span> can share this account.
            Each has their own isolated chat history.
          </p>
          ...
          <AnimatePresence mode="popLayout">
            <div style={{
              ...
              gap: isMobile ? 16 : 24,
              justifyContent: profileList.length === 0 ? "center" : "flex-start",
              ...
              {profileList.length === 0 && (
                <motion.p
                  ...
                  style={{
                    fontFamily: "Syne, sans-serif", fontSize: "0.8rem",
                    color: "rgba(232,217,160,0.25)", textAlign: "center", width: "100%",
                  }}>
          ...
          <div style={{
            ...
            borderTop: "1px solid rgba(244,180,0,0.06)",
            display: "flex", justifyContent: "space-between", alignItems: "center",
          }}>
            <span style={{
              fontFamily: "Orbitron, sans-serif", fontSize: "0.52rem",
              letterSpacing: "0.12em", color: "rgba(244,180,0,0.3)",
            }}>
            ...
            <div style={{ display: "flex", gap: 4 }}>
              {[...Array(MAX_PROFILES)].map((_, i) => (
                <div key={i} style={{
                  ...
                  background: i < profileList.length ? "rgba(244,180,0,0.6)" : "rgba(244,180,0,0.1)",
        ...
        <SectionCard title="Account Info" icon="✏️" delay={0.1}>
          <HexInput label="Name" value={username}
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginTop: "0.5rem" }}>
            <HexActionButton onClick={saveInfo} loading={infoLoading} disabled={infoLoading}>
              Save Info
            </HexActionButton>
          </div>
        </SectionCard>
        ...
        <SectionCard title="Change Password" icon="🔐" delay={0.2}>
          <HexInput label="Current Password" type="password" value={currentPw}
            onChange={e => setCurrentPw(e.target.value)} placeholder="••••••••••" />
          <HexInput label="New Password" type="password" value={newPw}
            onChange={e => setNewPw(e.target.value)} placeholder="••••••••••" />
          <HexInput label="Confirm New Password" type="password" value={confirmPw}
          ...
          {newPw && (
            <div style={{ marginBottom: "1.25rem", marginTop: "-0.5rem" }}>
              <div style={{ display: "flex", gap: 4, marginBottom: 4 }}>
                {[1, 2, 3, 4].map(i => {
                  const strength = Math.min(4,
                    ...
                    (/[A-Z]/.test(newPw) && /[0-9]/.test(newPw) ? 1 : 0)
                  );
                  const colors = ["#FF5555", "#FF9800", "#FFD54F", "#4ADE80"];
                  return (
                    <div key={i} style={{
                      flex: 1, height: 3, borderRadius: 2,
                      background: i <= strength ? colors[strength - 1] : "rgba(244,180,0,0.08)",
                      transition: "background 0.3s",
                    }} />
                  );
                })}
              ...
              <span style={{
                ...
                letterSpacing: "0.15em", color: "rgba(244,180,0,0.35)",
              }}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end" }}>
            <HexActionButton onClick={savePassword} loading={pwLoading} disabled={pwLoading}>
              Change Password
            </HexActionButton>
          </div>
        </SectionCard>
        ...
        <SectionCard title="Profession & Preferences" icon="🎯" delay={0.3}>
          <p style={{
            fontFamily: "Syne, sans-serif", fontSize: "0.82rem",
            color: "rgba(232,217,160,0.45)", marginBottom: "1.2rem", lineHeight: 1.6,
          }}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end" }}>
            <HexActionButton onClick={saveProfession} loading={profLoading} disabled={profLoading || !profession}>
              Save Preference
            </HexActionButton>
          </div>
        </SectionCard>
          ...
          <div style={{
            ...
            <div>
              <p style={{
                ...
                color: "rgba(232,217,160,0.55)", marginBottom: 4,
              }}>Signed in as <span style={{ color: "#F4B400" }}>{email || username}</span></p>
              <p style={{
                fontFamily: "Syne, sans-serif", fontSize: "0.72rem",
                color: "rgba(232,217,160,0.25)",
              }}>You will be redirected to the login page.</p>
            </div>
            <HexActionButton variant="danger" onClick={handleLogout}>
              ⏻ &nbsp;Logout
            </HexActionButton>
          </div>
        </SectionCard>
