...
export const auth = {
  ...
  register: (name, email, password, profession) =>
    request("/auth/register", { method: "POST", body: JSON.stringify({ name, email, password, profession }) }),
  ...
  logout: () => {
    ...
    localStorage.removeItem("mb_profiles_cache"); // clear cache on logout
  },
...
export const profiles = {
  ...
  create: (name, avatar, profession) =>
    request("/profiles", { method: "POST", body: JSON.stringify({ name, avatar, profession }) }),

  update: (profileId, updates) =>
    request(`/profiles/${profileId}`, { method: "PATCH", body: JSON.stringify(updates) }),
  ...
  getActive: () => {
    try { return JSON.parse(localStorage.getItem("mb_active_profile")); } catch { return null; }
  },
  ...
  // ── Cache helpers for sidebar switcher ──
  getCached: () => {
    try { return JSON.parse(localStorage.getItem("mb_profiles_cache") || "[]"); } catch { return []; }
  },
  setCached: (list) => {
    try { localStorage.setItem("mb_profiles_cache", JSON.stringify(list)); } catch {}
  },
};

export const chat = {
  create: (tone) =>
    profileRequest("/chats", { method: "POST", body: JSON.stringify({ tone }) }),

  sendMessage: (chatId, message, tone) =>
    profileRequest(`/chats/${chatId}/messages`, { method: "POST", body: JSON.stringify({ content: message, tone }) }),

  getHistory: () => profileRequest("/chats"),

  getConversation: (id) => profileRequest(`/chats/${id}`),

  deleteConversation: (id) => profileRequest(`/chats/${id}`, { method: "DELETE" }),

  rename: (chatId, title) =>
    profileRequest(`/chats/${chatId}`, { method: "PATCH", body: JSON.stringify({ title }) }),
};
...
export const user = {
  ...
  updateProfile: (updates) =>
    request("/user/profile", { method: "PATCH", body: JSON.stringify(updates) }),
};